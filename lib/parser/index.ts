...
export function parseStructuredPnr(raw: string): ParseResult {
  ...
  if (detection.gds === "sabre" || detection.gds === "galileo") {
    const result = detection.gds === "sabre" ? parseSabre(input) : parseGalileo(input);
    return {
      ...result,
  ...
  const code = detection.gds === "amadeus" ? "AMADEUS_GRAMMAR_PENDING" : detection.ambiguous ? "GDS_AMBIGUOUS" : "GDS_UNKNOWN";
  return {
    schemaVersion: 1, rawLength: input.rawLength,
    gds: field(detection.gds === "amadeus" ? "amadeus" : null, input.lines[0].span, "unverified", [code]),
    ...
    diagnostics: [{ code, severity: "blocking", source: detection.evidence, message: detection.gds === "amadeus" ? "Amadeus grammar is not implemented; review the original PNR." : "PNR format is unknown or ambiguous; review the original PNR." }],
