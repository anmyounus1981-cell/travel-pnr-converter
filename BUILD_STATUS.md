...
Invite-only agent login, password reset, sign-out and server checks for Converter, History, conversion CRUD and PDF are implemented locally. Migration 0003_owner_rls.sql closes the permissive database policies and enforces parent ownership on child rows; it has not been applied to the live Supabase project in this workspace. Public signup must also be disabled in the project Auth settings before the first agent invitation.
Phase 1 of the parser refactor adds typed source-aware records, a line-preserving lexer and golden Sabre/Galileo fixtures shared with the existing regression tests. The old converter remains in use until dedicated GDS grammars and integration are implemented; source spans, booking status and RBD are not yet exposed by live conversions. See docs/PARSER_PHASE1.md.
